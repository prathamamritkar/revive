    ...
    <header className="border-b border-[rgb(var(--color-line))] bg-[rgb(var(--color-card))] sticky top-0 z-50 shadow-xs backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-3 sm:gap-6">
          ...
          {/* Right: Consolidated Page Navigation Navbar & Settings right-aligned */}
          <div className="flex items-center justify-end gap-2 sm:gap-3 shrink-0 ml-auto min-w-0">
            <nav
              aria-label="Main Navigation"
              className="flex items-center overflow-x-auto py-1 scrollbar-none"
            >
              <div className="flex items-center gap-1 sm:gap-1.5 bg-[rgb(var(--color-surface))] p-1 sm:p-1.5 rounded-2xl border border-[rgb(var(--color-line))] shadow-xs">
                {tabs.map((tab, idx) => {
                  const Icon = tab.icon;
                  ...
                  return (
                    <button
                      ...
                      title={tab.label}
                      className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all duration-150 cursor-pointer select-none ${
                        isActive
            ...
            {/* System Settings Trigger & Popover Menu */}
